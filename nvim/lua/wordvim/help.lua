...
local table_rows = {
  ...
  'Enter / 1-9 / .           Style number / repeat last style (.); Space w r: styles',
...
local java_rows = {
  'RENAME AND NEW CLASSES', '----------------------',
  'Space jr / Space rn   Rename symbol under cursor; :wa saves all changes',
  'New empty *.java      Package and public class generated when opened',
  ':JavaClass            Generate skeleton in an empty Java buffer',
  'COMMENTS AND FORMATTING', '-----------------------',
  'gcc                   Toggle current line comment (Normal)',
  'V then j/k then gc    Select lines and toggle comments',
  'Space f / :w          Format now / on save; valid syntax required',
  ':ConformInfo          Check google-java-format / JDTLS availability',
  'Diagnostics update during typing when the server responds.',
  'JDTLS must finish importing the project; :WordJavaStatus checks it.',
  'JAVA NAVIGATION AND ACTIONS', '---------------------------',
...
local nvim_rows = {
  'NEO-TREE', '--------',
  'Space e               Open files; a creates file (include .java)',
  't                     Open selected file in new tab',
  '?                     Show Neo-tree shortcuts',
...
local linux_rows = {
  ...
  "-----------------------",
  "Prerequisites: JDK (javac -version) and Maven (mvn -v) on PATH.",
  "IntelliJ bundled Maven is not automatically available in the console.",
  "Use shell commands below outside the Neovim : command line.",
  "Create project (one line):",
  ...
  "For reportGenerator with Main in the default package:",
  "mvn compile exec:java \"-Dexec.mainClass=Main\"",
  "Keep the leading minus in -D. Include package name when present.",
  "Plain Java: create Main.java containing public static void main(String[] args).",
  "javac Main.java       Compile (without third-party dependencies)",
  "java Main             Run compiled class in current directory",
  "NODE PROJECTS",
  "-------------",
  "mkdir my-node-app     Then: cd my-node-app",
  "npm init -y           Create package.json",
  "Create index.js with: console.log(\"Hello\");",
  "npm pkg set scripts.start=\"node index.js\" scripts.test=\"node --test\"",
  "npm start / node index.js  Launch",
  "Create *.test.js using node:test and node:assert/strict.",
  "npm test              Run those tests (Node 20+)",
  "npm install           Install dependencies of existing project",
  "npm ci                Reproduce dependencies with package-lock.json",
  "npm run               List scripts; dev exists only if configured",
  "Ctrl+C                Stop foreground application",
  "Wrapper: ./mvnw test (if mvnw exists).",
  "SHELL COMMANDS", "--------------",
...
local powershell_rows = {
  ...
  "Wrapper: .\\mvnw.cmd test (if included).",
  "SHELL COMMANDS", "--------------",
